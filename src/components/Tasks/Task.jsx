...
const Task = ({ id, text, completed, onEdit, onRemove, onComplete }) => {
  ...
  const onEditTask = () => {
    const taskText = window
      .prompt("Введите новое название задачи", text)
      .trim();
    if (taskText) {
      axios
  ...
  return (
    <li key={id} className="tasks__items-block">
      <input
        id={`task-${id}`}
      ...
      <p>{text}</p>
      <div className="tasks__items-controls">
        <button className="tasks__items-button" onClick={onEditTask}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="15"
            height="15"
        ...
        <button className="tasks__items-button" onClick={onRemoveTask}>
          <svg
            width="10"
            height="10"
            viewBox="0 0 10 10"
